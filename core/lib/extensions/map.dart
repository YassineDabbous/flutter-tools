...
extension MapExtensions on Map {
  ...
  Map<String, dynamic> get toDynamic => map(
    (key, value) => MapEntry(
      key,
      int.tryParse(value) ??
  ...
  String get toQuery => map(
    (key, value) => MapEntry(key, value == null ? value : '$key=$value'),
  ).values.where((element) => element != null).join('&');
  ...
  Map get sorted => Map.fromEntries(
  ...
  Map<String, V> sort<String, V extends Object>() {
    final x = entries
        .map((e) => MapEntry(e.key.toString(), e.value as V))
        .toList();
