...
class AppWrapper extends StatelessWidget {
  ...
  void listener(BuildContext context, AuthenticationState state) {
    ...
    if (state is AuthenticationAuthenticated) {
      ...
      if (routes.guest.contains(navPath) || navPath == routes.accounts()) {
        ...
        Core.get<I>().activate(user: state.auth); // Activate app services ('I' is the Initializer abstract class)
        Core.nav.popUntil((r) => r.isFirst); // Clear the navigation history stack
        Core.nav.navigate(routes.redirectAfterLogin); // Redirect to the authenticated home path
      }
    } else if (state is AuthenticationLogout) {
      // Soft logout from current profile (profile token cleared, root account token remains).
      Core.get<I>().deactivate(); // Deactivate app services ('I' is the Initializer abstract class)
      if (!routes.guest.contains(navPath)) {
        // Redirect to the soft logout page if the user was on a protected page.
        logAuth.info('Redirecting to soft logout: ${routes.redirectAfterLogout}');
      ...
      if (!routes.guest.contains(navPath)) {
        ...
        logAuth.info('Redirecting to hard logout: ${routes.redirectAfterHardLogout}');
        Core.nav.pushReplacement(routes.redirectAfterHardLogout);
      }
    } else {
      throw UnimplementedError('AuthenticationState type not detected in AppWrapper listener.');
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return BlocListener<AuthenticationCubit, AuthenticationState>(listener: listener, child: child);
  }
}
